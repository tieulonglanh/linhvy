...
class QuestionController extends AppController {
	...
	*/
    public function index() {
		$this->paginate = array('limit'=>15, 'order'=>'Question.id DESC');
	...
	*/
	public function edit($id = null)
	{
		...
		if (!$id && empty($this->request->data)) {
            $this->redirect(array('controller'=>'Question', 'action'=>'index'));
        }else if (!empty($this->request->data)) {
            $data['Question'] = $this->request->data['Question'];
            if ($this->Question->save($data['Question'])){
	...
	*/
	public function delete_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Question->delete($condition)){
	...
	*/
	public function active($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            if($this->Question->save(array('Question'=>array('status'=>1, 'id'=>$id)))){ 
	...
	*/
	public function active_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Question->updateAll(array('status'=>1), array('Question.id'=>$condition))){
	...
	*/
	public function close($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
			$conditions = array('Question'=>array('status'=>0, 'id'=>$id));
            if($this->Question->save($conditions)){
	...
	*/
	public function close_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Question->updateAll(array('status'=>0), array('Question.id'=>$condition))){
	...
	*/
	public function search()
	{
		if($this->request->is('POST')){
			...
			$conditions = array(
						'or'=>array(
							'Question.name like "%'.$keyword.'%"',
							'Question.name_en like "%'.$keyword.'%"',
						)
					); 
			$this->paginate = array(
					'conditions'=>$conditions,
					'limit'=>15,
					'order'=>'Question.id DESC, Question.modified DESC'
			);
			$Question = $this->paginate('Question');
        	$this->set('Question', $Question);
			$this->render('index');
