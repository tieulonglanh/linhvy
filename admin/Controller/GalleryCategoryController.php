...
/*
**
** Class xử lý phần quản lý danh mục galley
...
class GalleryCategoryController extends AppController {
    ...
    public $uses = array('GalleryCategory');
	...
	*/
    public function index() {
		$this->paginate = array('limit'=>15, 'order'=>'GalleryCategory.id DESC');
		$gallery_category = $this->paginate('GalleryCategory');
	...
	*/
    public function add() {
        ...
        if (!empty($this->request->data)) {	
			if(empty($this->request->data['GalleryCategory']['parent_id'])) $this->request->data['GalleryCategory']['parent_id'] = 0;
        ...
        $list_cat = $this->GalleryCategory->generateTreeList(null,null,null," _ ");
	...
	*/
	public function edit($id = null)
	{
		...
		if (!$id && empty($this->request->data)) {
            $this->redirect(array('controller'=>'GalleryCategory', 'action'=>'index'));
        }else if (!empty($this->request->data)) {
            $data['GalleryCategory'] = $this->request->data['GalleryCategory'];
            if ($this->GalleryCategory->save($data['GalleryCategory'])){
	...
	*/
    public function delete($id = null) {
		$this->Session->setFlash(__('', true));
        if (!empty($id)){
            $this->GalleryCategory->delete($id);
	...
	*/
	public function delete_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->GalleryCategory->delete($condition)){
	...
	*/
	public function active($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->GalleryCategory->save(array('GalleryCategory'=>array('status'=>1, 'id'=>$id)));
	...
	*/
	public function active_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->GalleryCategory->updateAll(array('status'=>1), array('GalleryCategory.id'=>$condition))){
	...
	*/
	public function close($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->GalleryCategory->save(array('GalleryCategory'=>array('status'=>0, 'id'=>$id)));
	...
	*/
	public function close_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->GalleryCategory->updateAll(array('status'=>0), array('GalleryCategory.id'=>$condition))){
	...
	*/
	public function search()
	{
		if($this->request->is('POST')){
			...
			$conditions = array(
						'or'=>array(
							'GalleryCategory.name like "%'.$keyword.'%"',
							'GalleryCategory.name_en like "%'.$keyword.'%"',
						)
					); 
			$this->paginate = array(
					'conditions'=>$conditions,
					'limit'=>15,
					'order'=>'GalleryCategory.id DESC, GalleryCategory.modified DESC'
			);
			$gallery_category = $this->paginate('GalleryCategory');
        	$this->set('gallery_category', $gallery_category);
			$this->render('index');
