...
class PartnerController extends AppController {
	...
	*/
	public function edit($id = null)
	{
		...
		if (!$id && empty($this->request->data)) {
            ...
            $data['Partner'] = $this->request->data['Partner'];
	...
	*/
	public function copy($id)
	{
		$this->Session->setFlash(__('', true));
		if(!empty($id)){
			$data = $this->Partner->read(null, $id);
			$this->data = $data;
			$this->render('add');
		}else{
			$this->redirect(array('action' => 'index'));	
	...
	*/
	public function delete_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Partner->delete($condition)){
	...
	*/
	public function active($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            if($this->Partner->save(array('Partner'=>array('status'=>1, 'id'=>$id)))){ 
	...
	*/
	public function active_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Partner->updateAll(array('status'=>1), array('Partner.id'=>$condition))){
	...
	*/
	public function close($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
			$conditions = array('Partner'=>array('status'=>0, 'id'=>$id));
            if($this->Partner->save($conditions)){
	...
	*/
	public function close_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Partner->updateAll(array('status'=>0), array('Partner.id'=>$condition))){
	...
	*/
	public function search()
	{
		if($this->request->is('POST')){
			...
			$conditions = array(
						'or'=>array(
							'Partner.name like "%'.$keyword.'%"',
							'Partner.name_en like "%'.$keyword.'%"',
						)
					); 
			$this->paginate = array(
					'conditions'=>$conditions,
					'limit'=>15,
					'order'=>'Partner.id DESC, Partner.modified DESC'
			);
			$Partner = $this->paginate('Partner');
        	$this->set('Partner', $Partner);
			$this->render('index');
