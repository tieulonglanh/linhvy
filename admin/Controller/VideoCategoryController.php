...
**
*/
class VideoCategoryController extends AppController {

    public $name = 'VideoCategory';
    public $uses = array('VideoCategory');
	...
	*/
    public function index() {
		$this->paginate = array('limit'=>15, 'order'=>'VideoCategory.id DESC');
		$video_category = $this->paginate('VideoCategory');
        $this->set('video_category', $video_category);
    }
	...
	*/
    public function add() {
        ...
        if (!empty($this->request->data)) {	
			if(empty($this->request->data['VideoCategory']['parent_id'])) $this->request->data['VideoCategory']['parent_id'] = 0;
	...
	*/
	public function edit($id = null)
	{
		...
		if (!$id && empty($this->request->data)) {
            $this->redirect(array('controller'=>'VideoCategory', 'action'=>'index'));
        }else if (!empty($this->request->data)) {
            $data['VideoCategory'] = $this->request->data['VideoCategory'];
            if ($this->VideoCategory->save($data['VideoCategory'])){
            ...
            $this->data = $this->VideoCategory->read(null, $id);
        }
		$list_cat = $this->VideoCategory->generateTreeList(null,null,null," _ ");
        $this->set(compact('list_cat'));
	}
	...
	*/
	public function copy($id)
	{
		$this->Session->setFlash(__('', true));
		if(!empty($id)){
			$data = $this->VideoCategory->read(null, $id);
			$this->data = $data;
			$list_cat = $this->VideoCategory->generateTreeList(null,null,null," _ ");
        	$this->set(compact('list_cat'));
			$this->render('add');
	...
	*/
	public function delete_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->VideoCategory->delete($condition)){
	...
	*/
	public function active($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->VideoCategory->save(array('VideoCategory'=>array('status'=>1, 'id'=>$id)));
	...
	*/
	public function active_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->VideoCategory->updateAll(array('status'=>1), array('VideoCategory.id'=>$condition))){
	...
	*/
	public function close($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->VideoCategory->save(array('VideoCategory'=>array('status'=>0, 'id'=>$id)));
	...
	*/
	public function close_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->VideoCategory->updateAll(array('status'=>0), array('VideoCategory.id'=>$condition))){
	...
	*/
	public function search()
	{
		if($this->request->is('POST')){
			...
			$conditions = array(
						'or'=>array(
							'VideoCategory.name like "%'.$keyword.'%"',
							'VideoCategory.name_en like "%'.$keyword.'%"',
						)
					); 
			$this->paginate = array(
					'conditions'=>$conditions,
					'limit'=>15,
					'order'=>'VideoCategory.id DESC, VideoCategory.modified DESC'
			);
			$video_category = $this->paginate('VideoCategory');
        	$this->set('video_category', $video_category);
			$this->render('index');
