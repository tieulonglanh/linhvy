...
/*
**
** Class xử lý phần quản lý galley
...
class GalleryController extends AppController {
    ...
    public $name = 'Gallery';
    public $uses = array('Gallery', 'GalleryCategory');
	...
	*/
    public function add() {
        ...
        if (!empty($this->request->data)) {	
			...
			if(empty($data['Gallery']['gallery_category_id'])){
				$this->Session->setFlash(__('Chưa chọn danh mục Gallery !', true));
			}else{
				if(!empty($data['multi_images'])){
					$data['Gallery']['multi_images'] = implode("|", $data['multi_images']);
				}
	...
	*/
	public function edit($id = null)
	{
		...
		if (!$id && empty($this->request->data)) {
			...
			if(!empty($this->data['Gallery']['multi_images'])){
				$multi_images = explode("|", $this->data['Gallery']['multi_images']);
	...
	*/
	public function copy($id)
	{
		$this->Session->setFlash(__('', true));
		if(!empty($id)){
			$data = $this->Gallery->read(null, $id);
			$this->data = $data;
			if(!empty($this->data['Gallery']['multi_images'])){
				$multi_images = explode("|", $this->data['Gallery']['multi_images']);
				$this->set('multi_images', $multi_images);
	...
	*/
	public function delete_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Gallery->delete($condition)){
	...
	*/
	public function active($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->Gallery->save(array('Gallery'=>array('status'=>1, 'id'=>$id)));
	...
	*/
	public function active_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Gallery->updateAll(array('status'=>1), array('Gallery.id'=>$condition))){
	...
	*/
	public function close($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->Gallery->save(array('Gallery'=>array('status'=>0, 'id'=>$id)));
	...
	*/
	public function close_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Gallery->updateAll(array('status'=>0), array('Gallery.id'=>$condition))){
		...
		if($this->request->is('POST')){
			...
			$conditions = array(
						'or'=>array(
							...
							'Gallery.name_en like "%'.$keyword.'%"',
						)
					); 
			$this->paginate = array(
					'conditions'=>$conditions,
					'limit'=>15,
					'order'=>'Gallery.id DESC, Gallery.modified DESC'
			);
			$gallery = $this->paginate('Gallery');
        	$this->set('gallery', $gallery);
			$this->render('index');
