...
class SupportController extends AppController {
	...
	*/
    public function index() {
		$this->paginate = array('limit'=>15, 'order'=>'Support.id DESC');
		$Support = $this->paginate('Support');
	...
	*/
    public function add() {
        ...
        if (!empty($this->request->data)) {	
			if(!empty($this->request->data['Support']['position'])){
	...
	*/
	public function edit($id = null)
	{
		...
		if (!$id && empty($this->request->data)) {
            $this->redirect(array('controller'=>'Support', 'action'=>'index'));
        }else if (!empty($this->request->data)) {
            $data['Support'] = $this->request->data['Support'];
	...
	*/
	public function copy($id)
	{
		$this->Session->setFlash(__('', true));
		if(!empty($id)){
			$data = $this->Support->read(null, $id);
			$this->data = $data;
			$this->render('add');
		}else{
			$this->redirect(array('action' => 'index'));	
	...
	*/
	public function delete_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Support->delete($condition)){
	...
	*/
	public function active($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            if($this->Support->save(array('Support'=>array('status'=>1, 'id'=>$id)))){ 
	...
	*/
	public function active_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Support->updateAll(array('status'=>1), array('Support.id'=>$condition))){
	...
	*/
	public function close($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
			$conditions = array('Support'=>array('status'=>0, 'id'=>$id));
            if($this->Support->save($conditions)){
	...
	*/
	public function close_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Support->updateAll(array('status'=>0), array('Support.id'=>$condition))){
	...
	*/
	public function search()
	{
		if($this->request->is('POST')){
			...
			$conditions = array(
						'or'=>array(
							'Support.name like "%'.$keyword.'%"',
							'Support.name_en like "%'.$keyword.'%"',
						)
					); 
			$this->paginate = array(
					'conditions'=>$conditions,
					'limit'=>15,
					'order'=>'Support.id DESC, Support.modified DESC'
			);
			$Support = $this->paginate('Support');
        	$this->set('Support', $Support);
			$this->render('index');
